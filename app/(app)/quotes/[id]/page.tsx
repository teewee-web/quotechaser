...
import { QuotePdfLoader } from "@/components/quote-pdf-loader";
...
async function logoDataUrl(supabase: Awaited<ReturnType<typeof createClient>>, storedPath?: string | null) {
  ...
  const { data } = await supabase.storage.from("business-logos").createSignedUrl(storedPath, 3600);
  if (!data?.signedUrl) return null;
  try {
    const response = await fetch(data.signedUrl, { cache: "no-store" });
    if (!response.ok) return null;
    const bytes = Buffer.from(await response.arrayBuffer());
    return `data:${response.headers.get("content-type") || "image/png"};base64,${bytes.toString("base64")}`;
  } catch {
    return null;
  }
}
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: quote }, { data: settings }] = await Promise.all([
    supabase.from("quotes").select("*,customers(*),quote_items(*)").eq("id", id).order("sort_order", { referencedTable: "quote_items" }).single(),
    supabase.from("user_settings").select("*").maybeSingle(),
  ]);
  if (!quote) notFound();
  const customer = Array.isArray(quote.customers) ? quote.customers[0] : quote.customers;
  if (!customer) notFound();
  const logoUrl = await logoDataUrl(supabase, settings?.logo_url);
  ...
  return <>
    <PageHeader title={quote.quote_number || "Quote"} description={`${customer.name} · Valid until ${ukDate(quote.expiry_date)}`}/>
    <div className="grid gap-5 lg:grid-cols-[1fr_auto]"><section className="card qc-card overflow-hidden"><div className="bg-[#102522] p-6 text-white"><p className="text-sm font-bold text-teal-200">QUOTATION TOTAL</p><p className="mt-2 text-4xl font-black text-amber-300">{gbp(quote.final_total || quote.value_pence)}</p></div><div className="grid gap-5 p-6 sm:grid-cols-2"><div><p className="label">Customer</p><p className="font-bold">{customer.name}</p><p className="muted whitespace-pre-line">{customer.address || "No address supplied"}</p></div><div><p className="label">Work</p><p>{quote.job_description}</p></div><div><p className="label">Quote date</p><p>{ukDate(quote.quote_date)}</p></div><div><p className="label">Status</p><span className={`badge ${quote.status}`}>{quote.status}</span></div></div></section><aside className="card qc-card flex min-w-64 flex-col gap-3 p-5"><QuotePdfLoader data={{ quote, customer, settings: completeSettings, logoUrl }}/><Link className="btn btn-secondary" href={`/quotes/${id}/edit`}><Pencil size={18}/>Edit quote</Link><Link className="btn btn-secondary" href="/quotes">Back to quotes</Link></aside></div>
  </>;
}
