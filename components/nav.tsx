...
export function PageHeader({ title, description, action }: { title: string; description?: string; action?: { href: string; label: string } }) {
  const path = usePathname();
  if (path === "/dashboard") return <header className="mb-7 flex items-start justify-between gap-4"><div><p className="mb-1 text-xs font-extrabold uppercase tracking-[.16em] text-teal-700">QuoteAlign</p><h1 className="text-3xl font-black tracking-[-.035em] text-[#102522] sm:text-4xl">{title}</h1>{description && <p className="muted mt-2 max-w-xl leading-6">{description}</p>}</div>{action && <Link href={action.href} className="btn btn-primary shrink-0"><Plus size={20} /><span className="hidden sm:inline">{action.label}</span></Link>}</header>;
  return <header className="page-banner"><div className="relative z-10"><p className="mb-2 text-xs font-black uppercase tracking-[.18em] text-amber-300">QuoteAlign workspace</p><h1>{title}</h1>{description && <p>{description}</p>}</div>{action && <Link href={action.href} className="btn relative z-10 shrink-0 border border-amber-300 bg-amber-300 text-[#102522] shadow-lg hover:bg-amber-200"><Plus size={20} /><span>{action.label}</span></Link>}</header>;
}
